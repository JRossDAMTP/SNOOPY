...
*/
/***************************************************************/
double newdt(struct Field fldi, double tremap, double t) {
...
#ifdef PULSE_KAPPA
	gamma_v += ((kxmax+fabs(tremap)*kymax)*(kxmax+fabs(tremap)*kymax)+kymax*kymax+kzmax*kzmax) * nu_th*(1+0.5*arctan(sin(t/param.th_period)/0.1));
#else
